#!/usr/bin/env python3
"""
Media Change Event Detection Verification

PURPOSE:
    Creates visualizations to verify the accuracy of automated media change
    event detection. Overlays detected events on raw oxygen consumption data
    for visual inspection and validation.

METHODOLOGY:
    - Loads raw oxygen consumption time series for selected wells
    - Overlays detected media change events as vertical lines
    - Shows both improved and original detection for comparison
    - Creates multi-panel plots for different plates/drugs
    - Highlights detection improvements and timing corrections

INPUTS:
    - Database connection via DATABASE_URL environment variable
    - results/data/improved_media_change_events.parquet
      Improved event detection results
    - results/data/media_change_events.parquet (optional)
      Original detection for comparison

OUTPUTS:
    - results/figures/event_verification/plate_*_event_verification.png
      Per-plate visualizations with 12 example wells each
    - results/figures/event_verification/spike_characterization_detailed.png
      Detailed view of spike characteristics
    - Console output with detection statistics

REQUIREMENTS:
    - numpy, pandas, matplotlib, duckdb
    - Media change events must be detected first
    - Database connection for raw oxygen data
"""
...
max_plates = min(10, len(plates_with_events))  # Show up to 10 plates
for plate_idx, plate_id in enumerate(plates_with_events[:max_plates]):
    print(f"\n🔍 Processing plate {plate_idx + 1}/{max_plates}: {plate_id}")
